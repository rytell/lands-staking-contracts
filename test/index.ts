...
describe("Stake Lands", function () {
  ...
  it("Should let a staker add a land to a hero remove it and add it to another hero", async () => {
    // whitelist collections
    for (let index = 0; index < LAND_COLLECTIONS_TO_DEPLOY; index++) {
      await expect(stakeLands.addLandCollection(landCollections[index].address))
        .not.to.be.reverted;
    ...
    const landsForHeroTwo = [];
    const collectionsForHeroTwo = [];

    // stake into another hero
    for (let collectionIndex = 0; collectionIndex < 1; collectionIndex++) {
      collectionsForHeroTwo.push(landCollectionsToRemove[collectionIndex]);
      landsForHeroTwo.push(landsToRemove[collectionIndex]);
    }
    ...
    await expect(
      stakeLands.addLandsToHero(
        herosOnWallet[1],
        [landCollectionsToRemove[1]],
        [landsToRemove[1]]
      )
    ).not.to.be.reverted;

    const resultingHeroOneLands = [];
    const resultingHeroTwoLands = [];
    let index = 0;
    while (true) {
      try {
        const accountLand = await stakeLands.stakedLands(owner.address, index);
        if (
          accountLand.heroId.toNumber() === herosOnWallet[0].toNumber() &&
          accountLand.staked
        ) {
          resultingHeroOneLands.push(accountLand);
        }

        if (
          accountLand.heroId.toNumber() === herosOnWallet[1].toNumber() &&
          accountLand.staked
        ) {
          resultingHeroTwoLands.push(accountLand);
        }
      } catch (error) {
        break;
      }
    ...
    expect(resultingHeroOneLands.length).to.equal(1);
    expect(resultingHeroTwoLands.length).to.equal(2);
  });
