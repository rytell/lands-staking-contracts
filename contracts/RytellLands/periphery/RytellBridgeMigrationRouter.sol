...
import "./libraries/RytellLibrary.sol";

contract RytellBridgeMigrationRouter {
    ...
    modifier ensure(uint256 deadline) {
        require(
            ...
            "RytellBridgeMigrationRouter: EXPIRED"
        );
        _;
    }
    ...
    modifier onlyAdmin() {
        require(
            ...
            "RytellBridgeMigrationRouter: Unauthorized"
        );
        _;
    }
    ...
    function addAdmin(address account) external onlyAdmin {
        require(
            account != address(0),
            "RytellBridgeMigrationRouter: Address 0 not allowed"
        );
    ...
    function removeAdmin(address account) external onlyAdmin {
        require(
            msg.sender != account,
            "RytellBridgeMigrationRouter: You can't demote yourself"
        );
        ...
        onlyAdmin
    {
        require(
            ...
            "RytellBridgeMigrationRouter: tokenAddress 0 not supported"
        );
        require(
            migratorAddress != address(0),
            "RytellBridgeMigrationRouter: migratorAddress 0 not supported"
        );
    ...
    function _allowToken(address tokenAddress, address spenderAddress)
        internal
    {
        IRytellERC20(tokenAddress).approve(spenderAddress, type(uint256).max);
    }
    ...
    {
        (uint112 reserve0, uint112 reserve1, ) = IRytellPair(pairToken)
            .getReserves();
        uint256 quote0 = amountIn0;
        uint256 quote1 = RytellLibrary.quote(amountIn0, reserve0, reserve1);
        if (quote1 > amountIn1) {
            quote1 = amountIn1;
            quote0 = RytellLibrary.quote(amountIn1, reserve1, reserve0);
        }
        ...
        liquidityAmount = IRytellPair(pairToken).mint(to);
    }
        ...
        returns (uint256 amountTokenA, uint256 amountTokenB)
    {
        ...
        (amountTokenA, amountTokenB) = IRytellPair(liquidityPair).burn(
            address(this)
        );
    }
    ...
    function _arePairsCompatible(address pairA, address pairB) internal view {
        require(
            pairA != address(0),
            "RytellBridgeMigrationRouter: liquidityPairFrom address 0"
        );
        require(
            pairA != address(0),
            "RytellBridgeMigrationRouter: liquidityPairTo address 0"
        );
        require(
            pairA != pairB,
            "RytellBridgeMigrationRouter: Cant convert to the same liquidity pairs"
        );
        require(
            IRytellPair(pairA).token0() == IRytellPair(pairB).token0() ||
                IRytellPair(pairA).token0() == IRytellPair(pairB).token1() ||
                IRytellPair(pairA).token1() == IRytellPair(pairB).token0() ||
                IRytellPair(pairA).token1() == IRytellPair(pairB).token1(),
            "RytellBridgeMigrationRouter: Pair does not have one token matching"
        );
    }
    ...
    function _migrateToken(address tokenAddress, uint256 amount) internal {
        ...
        require(
            ...
            "RytellBridgeMigrationRouter: Didn't yield the correct amount"
        );
    }
    ...
    function migrateToken(
        ...
        require(
            ...
            "RytellBridgeMigrationRouter: migrator not registered"
        );
        TransferHelper.safeTransferFrom(
            token,
            msg.sender,
    ...
    function migrateLiquidityWithPermit(
        ...
        IRytellPair(liquidityPairFrom).permit(
            msg.sender,
            address(this),
            amount,
            deadline,
    ...
    function _migrateLiquidity(
        ...
        address tokenToMigrate = IRytellPair(liquidityPairFrom).token0();
        if (
            IRytellPair(liquidityPairFrom).token0() ==
            IRytellPair(liquidityPairTo).token0() ||
            IRytellPair(liquidityPairFrom).token0() ==
            IRytellPair(liquidityPairTo).token1()
        ) {
            tokenToMigrate = IRytellPair(liquidityPairFrom).token1();
        }
        address newTokenAddress = bridgeMigrator[tokenToMigrate];
        require(
            newTokenAddress != address(0),
            "RytellBridgeMigrationRouter: Migrator not registered for the pair"
        );
        require(
            newTokenAddress == IRytellPair(liquidityPairTo).token0() ||
                newTokenAddress == IRytellPair(liquidityPairTo).token1(),
            "RytellBridgeMigrationRouter: Pair doesn't match the migration token"
        );
        ...
        );
        {
            ...
            if (tokenToMigrate != IRytellPair(liquidityPairFrom).token0()) {
        ...
        if (
            IRytellPair(liquidityPairFrom).token0() !=
            IRytellPair(liquidityPairTo).token0() &&
            IRytellPair(liquidityPairFrom).token1() !=
            IRytellPair(liquidityPairTo).token1()
        ) {
    ...
    {
        (uint112 reserves0, uint112 reserves1, ) = IRytellPair(pairAddress)
            .getReserves();
        uint256 totalSupply = IRytellPair(pairAddress).totalSupply();
    ...
    function calculateChargeBack(
        ...
        require(
            ...
            "RytellBridgeMigrationRouter: liquidityPairFrom address 0 not supported"
        );
        require(
            liquidityPairTo != address(0),
            "RytellBridgeMigrationRouter: liquidityPairTo address 0 not supported"
        );
        ...
        (uint112 reserve0, uint112 reserve1, ) = IRytellPair(liquidityPairTo)
            .getReserves();
        uint256 quote0 = amountIn0;
        uint256 quote1 = RytellLibrary.quote(amountIn0, reserve0, reserve1);
        if (quote1 > amountIn1) {
